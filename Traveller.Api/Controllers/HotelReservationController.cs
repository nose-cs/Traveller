...
public class HotelReservationController : ControllerBase
{
    ...
    public ActionResult GetSales([FromQuery] SalesRequest request)
    {
        ...
        {
            case GroupBy.Day:
                return Ok(_repositories.HotelReservations.Find().Where(reservation => DateOnly.FromDateTime(reservation.ArrivalDate) >= request.Start && DateOnly.FromDateTime(reservation.ArrivalDate) <= request.End)
