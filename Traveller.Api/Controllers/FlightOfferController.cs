...
public class FlightOfferController : ControllerBase
{
    ...
    public ActionResult GetSales([FromQuery] SalesRequest request)
    {
        ...
        return Ok(_repository.FlightReservations.FindWithInclude(reservation => reservation.Offer).Where(reservation => reservation.Offer.AgencyId == agencyId && DateOnly.FromDateTime(reservation.ArrivalDate) >= request.Start && DateOnly.FromDateTime(reservation.ArrivalDate) <= request.End)
